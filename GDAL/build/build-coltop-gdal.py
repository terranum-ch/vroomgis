...
def buildUnix(gdalpath):
    ...
    doSysLib = lsutilities.askUserWithCheck(
        "Use sytem libs (s), framework (f) or user defined path (p) for GEOS and PROJ ? ",
        ["s", "S", "f", "F", "p", "P"])
    ...
    if (isSucess == True):
        if (lsutilities.askUserWithCheck("Build GDAL now ?: ") == 'Y'):
            lsutilities.runProcess("make --jobs={}; make install".format(lsutilities.CountProcessor()), gdalpath,
                                   "Building GDAL with C2D support")
