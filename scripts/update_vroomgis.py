...
def buildCommand(name, version):
    ...
    myProcess = subprocess.Popen(gBuildCommand(version, os.path.join(gDirBin, name)), cwd=os.path.join(gDirBin, name),
                                 shell=False)
...
if (doBuildVroomloader == 'Y'):
    ...
    try:
        myProcess = subprocess.Popen(mycmakeCommandLine, 0, None, None, None, None, None, False, False,
                                     gDirBin + os.sep + "vroomloader")
        myProcess.wait()
        if (doClean == 'Y'):
            print ("\n****Configuring again to be sure after a clean****")
            myProcess2 = subprocess.Popen(mycmakeCommandLine, 0, None, None, None, None, None, False, False,
                                          gDirBin + os.sep + "vroomloader")
            myProcess2.wait()
    except:
...
if (doBuildVroomgistests == 'Y'):
    ...
    myProcess = subprocess.Popen(gBuildCommand("Debug", gDirBin + os.sep + "vroomgistests"), 0, None, None, None, None,
                                 None, False, False, gDirBin + os.sep + "vroomgistests")
    myRetcode = myProcess.wait()
    if (myRetcode != 0):
